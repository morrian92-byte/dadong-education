...
export default function TeacherRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.teacherRegister({ username, password, name, phone })
      setDone(true)
    } catch (err: any) {
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 w-full max-w-md text-center">
          ...
          <p className="text-slate-500 mt-2">你的教师账号正在等待管理员审批，审批通过后即可登录。</p>
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">教师注册</h1>
        <p className="text-slate-500 text-sm mb-6">注册后需等待管理员审批</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              className="w-full px-4 py-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" required />
          </div>
          <div>
